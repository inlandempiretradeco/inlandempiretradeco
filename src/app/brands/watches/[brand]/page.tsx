...
export default async function WatchBrandPage({ params }: { params: { brand: string } }) {
  ...
  return (
    <div className="max-w-[1320px] mx-auto px-6 py-14 lg:px-[72px] lg:py-20">
      ...
      {/* Stats grid */}
      <Reveal delay={0.1}>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-px mt-12 bg-white/5">
          {[
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {lotted.map((w, i) => (
                <Reveal key={w._id} delay={i * 0.06}>
                  <ProductCard
                    href={`/watches/${w.slug}`}
                    lotNumber={w.lotNumber} brand={w.brand} title={w.model}
          ...
          <Reveal>
            <div className="py-16 text-center">
              <p className="font-display text-[26px] font-light text-white/45 mb-4">
                No {brand.name} pieces in stock right now.
              </p>
              <p className="text-[14px] font-light text-white/32 max-w-[400px] mx-auto mb-10">
                Inventory moves quickly. Reach out and we can let you know what's coming in,
                or source a specific reference for you.
              </p>
        ...
        <Reveal>
          ...
          <div className="flex flex-wrap gap-2 mt-5">
            {watchBrands.filter(b => b.slug !== brand.slug).map(b => (
              <Link key={b.slug} href={`/brands/watches/${b.slug}`}
