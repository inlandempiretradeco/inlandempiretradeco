...
import { watchesByBrandQuery } from "@/sanity/lib/queries";
import { withLotNumbers, type Watch } from "@/lib/filters";
import { watchBrands, allWatchBrandSlugs, getBrand } from "@/lib/brands";
...
export async function generateMetadata({ params }: { params: Promise<{ brand: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: `${brand.name} watches — new and consignment — at Inland Empire Trading Co., Medical Lake, WA. Ships anywhere in the US.`,
  };
}
...
export default async function WatchBrandPage({ params }: { params: Promise<{ brand: string }> }) {
  ...
  let watches: Watch[] = [];
  try { watches = await client.fetch(watchesByBrandQuery, { brand: brand!.name }, { next: { revalidate } }); } catch {}
  const lotted = withLotNumbers(watches);
  ...
  return (
    <div className="max-w-[1320px] mx-auto px-6 py-14 lg:px-[72px] lg:py-20">
      <div className="flex items-center gap-3 mb-12">
        ...
        <Link href="/watches" className={`${mono} text-[8px] hover:text-white/60 transition-colors`}>Watches</Link>
        <span className="text-white/15 text-[10px]">→</span>
      ...
      <div className="mt-14 pt-12" style={bdr}>
        <Reveal>
          <p className={mono}>Notable References</p>
        ...
        {lotted.length > 0 ? (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {lotted.map((w, i) => (
                <Reveal key={w._id} delay={i * 0.06}>
                  <ProductCard href={`/watches/${w.slug}`} lotNumber={w.lotNumber} brand={w.brand} title={w.model}
                    meta={[w.caseMaterial, w.caseSize, w.movement].filter(Boolean).join(" · ")}
                    price={w.price} status={w.status} photo={w.photos?.[0]} />
                </Reveal>
              ))}
            </div>
          </>
          ...
          <Reveal>
            <div className="py-16 text-center">
              <p className="font-display text-[26px] font-light text-white/45 mb-4">No {brand!.name} pieces in stock right now.</p>
              <p className="text-[14px] font-light text-white/32 max-w-[400px] mx-auto mb-10">Reach out and we can let you know what's coming in, or source a specific reference for you.</p>
              <ContactBar itemLabel={`${brand!.name} watch`} />
            </div>
          </Reveal>
        )}
      </div>
      <div className="mt-16 pt-12" style={bdr}>
        <Reveal>
          <p className={mono}>Other Watch Brands</p>
          <div className="flex flex-wrap gap-2 mt-5">
            {watchBrands.filter(b => b.slug !== brand!.slug).map(b => (
              <Link key={b.slug} href={`/brands/watches/${b.slug}`} className="px-4 py-3 font-mono text-[9px] tracking-[0.28em] uppercase text-white/35 transition-colors hover:text-white/70 font-light" style={{ border: "1px solid rgba(255,255,255,0.06)" }}>{b.name}</Link>
            ))}
          </div>
        </Reveal>
      </div>
