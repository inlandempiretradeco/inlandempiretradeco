...
export async function generateMetadata({ params }: { params: { brand: string } }): Promise<Metadata> {
  ...
  return {
    ...
    description: `${brand.name} watches — new and consignment — available by inquiry at Inland Empire Trading Co. in Medical Lake, WA. Ships anywhere in the US.`,
  };
}
...
export default async function WatchBrandPage({ params }: { params: { brand: string } }) {
  ...
  let watches: Watch[] = [];
  try {
    watches = await client.fetch(watchesByBrandQuery, { brand: brand.name }, { next: { revalidate } });
  } catch {}
  ...
  const schema = {
    ...
    "@type": "CollectionPage",
    name: `${brand.name} Watches — Inland Empire Trading Co.`,
    description: brand.blurb,
    url: `https://www.inlandempiretradingco.com/watches/brands/${brand.slug}`,
  };
  ...
  return (
    <div className="max-w-[1320px] mx-auto px-6 py-14 lg:px-[72px] lg:py-20">
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 64 }}>
        <Link href="/watches" style={{ ...S.mono, fontSize: 8 }}>Watches</Link>
          ...
          <Reveal>
            <div style={{ padding: "80px 0", textAlign: "center" }}>
              ...
              <p style={{ fontSize: 14, fontWeight: 300, color: "rgba(255,255,255,0.28)", maxWidth: 420, margin: "0 auto 40px" }}>
                Inventory moves quickly. Reach out and Ramon can let you know what's coming in, or source a specific reference for you.
              </p>
